...
## Investigated and closed (2026-08-07): agent sandbox couldn't pull BANKING77/SNIPS/MASSIVE
Two independent blockers observed on the restricted development network, neither
fixable from inside the sandbox:
1. **No direct network route to `blob.core.windows.net`** (DNS doesn't resolve
   without an outbound proxy) -- that's the CDN Xet/LFS-backed repos redirect
   large files to, and the outbound proxy returned 407 on those redirects.
2. **The available HF mirror endpoint only proxied MODEL repos, not dataset
   repos.** Model downloads (e.g. `sentence-transformers/all-MiniLM-L6-v2`)
   succeeded through it; every dataset repo tried (`repo_type="dataset"`)
   returned 401, including via the `hf_hub_download()` file-level API.

**Conclusion:** a sandbox-network limitation, not a code or credentials
problem. Fixed by downloading on an unrestricted network and handing off the
files locally -- see `../experiments/code/data/README.md` for the
manual-download workflow and the local-file loaders already built for it.
