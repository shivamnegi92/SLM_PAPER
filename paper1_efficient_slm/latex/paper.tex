...
construction). \textbf{(C3)} a real, measured implicit-slot scope statement (0\%
on our benchmarks) that honestly bounds where span-based heads suffice.
\textbf{(C4)} a negative-results study of common CPU inference optimizations and their root causes. We position our contribution honestly:
...
\caption{Linear-probe intent accuracy vs.\ backbone depth, all five datasets.
Every curve is nearly flat: probe accuracy saturates by depth 3, motivating the
a-priori depth-3 pick before any fine-tuning.}
\end{figure}
...
\textbf{C3: implicit slots (scope statement).} A slot value is
